...
✅ Milestone B: Build Execution Infrastructure + docs (ARCHITECTURE, ROADMAP)
    |                Complete. 29/29 tasks. B rework done: B2.3, B3.2, B4.0/4.1, B4.5, B4.6.
    |
...
**Status**: **COMPLETE** — All 29/29 tasks done (B1–B5, including B rework: B2.3, B3.2, B4.0/4.1, B4.5, B4.6). 100/101 unit tests pass (1 pre-existing). Shadow mode verified on VPS. ARCHITECTURE.md and ROADMAP.md written.
...
| ✅ **C4.2: Proactive near-resolution exit** | Implemented in `orchestrator::check_proactive_exits()` using `PROACTIVE_EXIT_MULTIPLIER = 1.2`. **INC-020 (2026-04-27)**: split into separate live and shadow execution paths. Live mode submits real FAK SELL orders via `Engine::execute_position_exit` (built on `Executor::execute_arb` with `Side::Sell`), polls `Executor::sell_orders_for_position` up to ~3s, then reconciles bookkeeping from actual fills via `Engine::apply_exit_fills` — partial fills reduce shares in place, zero fills leave the position open (no phantom proceeds). Shadow mode retains the legacy `liquidate_position` paper-bookkeeping path. Telegram notification on each submitted exit. The 1.2 multiplier is a hardcoded constant; empirical calibration against the value-curve and bid/ask depth is a known open follow-up. |
...
| Task | Acceptance Criteria |
|------|-------------------|
| 🔧 **E4: Run for 14 consecutive days** | IN PROGRESS — started 2026-03-24 14:16 UTC (day 5 of 14). All 6 instances: zero unhandled errors. Auto-recover from transient failures. v0.20.0 added `evaluated_opportunities` SQLite table + strategy eval counters for post-run threshold calibration. After 16h: zero opportunities at 1% threshold (market quiet). v0.20.1 fixed strategy-only positions not being resolution-checked (INC-015) — Wellington/Montedio stuck open 13+ hrs. |
...
|------|-------------------|--------|
| ⬚ **F-pre-1: Wire executor into orchestrator** | Instantiate `Executor` in `Orchestrator::new()`. Call `executor.execute_arb()` after `engine.enter_position()`. Add executor config block. Implement live-mode kill switch via `executor.cancel_all_orders()`. Wire unwind path: when `evaluate_arb_fills()` returns `Unwind`, executor sells filled legs. D9 validates decision logic only — unwind execution depends on this gate. | Risk analysis + D9 |
